...
class Helpers {
	...
	public static function locate_template( $tpl ) {
		...
		$path = apply_filters( 'BEA/Helpers/locate_template/templates', array( 'views/' . BEA_PROJECTHUDDLE_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( BEA_PROJECTHUDDLE_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( BEA_PROJECTHUDDLE_DIR . 'views/' . $tpl . '.php' );
		}
